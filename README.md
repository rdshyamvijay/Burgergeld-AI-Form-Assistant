# Bürgergeld AI Form Assistant (Laravel 12 + Gemini + PDFtk)
...
- **Household annex planner**: toggling “another adult in my BG” feeds the annex selector and prepares WEP data.
...
- **Field-level assistant**: every “?” button opens a chat widget that:
  ...
  - lets users ask follow‑ups with conversation context,
...
- Uses `mikehaertl/pdftk` + system `pdftk` to populate `storage/app/forms/Bürgergeld_Hauptantrag.pdf` and `storage/app/forms/WEP.pdf`.
...
├── app/
│   ├── Http/Controllers/
│   │   ├── AiController.php         # AI endpoints (annex planner, WEP questions, chat)
│   │   └── CitizenFormPdfController.php  # HA + WEP fill + merge
│   └── Services/GeminiClient.php   # REST helper (text, JSON, and chat)
├── config/
│   ├── ai.php                      # Gemini configuration (model, endpoint, temperature)
│   ├── ha.php                      # HA field schema (drives the UI)
│   ├── wep.php                     # Required WEP keys + enum lists
│   └── annex.php                   # Deprecated safety copy of earlier rules (optional)
├── resources/views/
│   ├── layouts/app.blade.php       # Shared layout + floating chat widget
│   ├── apply.blade.php             # HA form + household card
│   ├── annexes.blade.php           # Annex summary pills
│   └── wep.blade.php               # Missing WEP fields form
├── routes/
│   ├── web.php                     # UI routes
│   └── api.php                     # JSON + AI endpoints
├── storage/app/forms/
│   ├── Bürgergeld_Hauptantrag.pdf
│   └── WEP.pdf
└── README.md
```
...
| Gemini API key | — | Set `GEMINI_API_KEY` in `.env`; fallback messages still work without it |
...
cp /path/to/Bürgergeld_Hauptantrag.pdf storage/app/forms/
...
1. **Go to `/apply`**  
   - Fill applicant details; click “?” anytime to see AI guidance.  
   - Toggle “I live with another adult in my BG” to trigger WEP.
...
Legacy PDF API (`POST /api/citizen-form-pdf`) still exists for headless HA filling, but the recommended path is `/apply` → `/merged`.
...
|---------------------|-------------------|
| Annex planner       | “Always include HA… include WEP for 15+ BG members…” + domain JSON |
| WEP question builder| List of `{key,label,type,options,current_value}` → “Return ONLY JSON {questions:…}” |
| Field tooltip       | Field label/type/required/options → “Explain in <= 2 sentences…” |
...
Or stay in the UI: `php artisan serve` → `http://127.0.0.1:8000/apply`.

---

## 📄 License & Credits

- **License:** MIT – free to adapt for your own Bürgergeld automation experiments.
- **Built with:** Laravel 12, PHP 8.3, PDFtk, Google Gemini, Tailored prompt engineering.
- **Maintainer:** Shyamkumar Selvakumar

Questions or ideas? Open an issue or start a discussion on the repository. Viel Erfolg beim Ausfüllen! 📝
